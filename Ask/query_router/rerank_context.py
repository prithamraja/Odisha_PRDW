...
FAMILY_DESCRIPTIONS: dict[str, dict] = {
    ...
    'alr_011__progress_exceptions': {
        "desc": 'RANKS: Which blocks in a given district are below a given threshold percent activity completion in a given year. Returns activities, completed, started, completion_rate_pct. One row per block_name × district_name by default; other breakdowns on request. Filterable by district; answers state-wide when no place is named — one entry serves every scope. Caveat: There is no mid-year cut-off in the data, so the whole year is measured.',
        "members": ['ALR-011'],
    },
    ...
    'ast_001__asset_creation': {
        "desc": "COUNTS: How many assets were created in a given gram panchayat during a given year. Returns asset_rows, asset_creating_activities, categorised_assets. One row per gp_name × block_name by default; other breakdowns on request. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. 'District-wise / summary of asset creation' is THIS with the district breakdown, not AST-002 (category split in a block) or AST-003 (one sub-category). Caveat: activity_asset is sparsely populated: asset_category has values on 4,286 of 12,704 rows and asset_subcategory on 4,286;.",
        "members": ['AST-001'],
    },

    'ast_002__asset_creation': {
        "desc": 'COUNTS: What is the asset category-wise count of assets created in a given block for a given year. Returns asset_category_label, asset_rows, activities, expenditure. One row per asset_category_label. expenditure is actual expenditure on the PLAN basis (activity_expenditure), not the cashbook. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Splits assets by CATEGORY. A district-wise or overall summary of asset creation is AST-001. Caveat: activity_asset is sparsely populated: asset_category has values on 4,286 of 12,704 rows and asset_subcategory on 4,286;.',
        "members": ['AST-002'],
    },

    'ast_003__asset_creation': {
        "desc": 'COUNTS: How many a given asset sub-category assets exist across a given district for a given year. Returns asset_subcategory_label, asset_rows, gps, expenditure. expenditure is actual expenditure on the PLAN basis (activity_expenditure), not the cashbook. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Counts ONE named asset sub-category. A district-wise summary of all asset creation is AST-001. Caveat: activity_asset is sparsely populated: asset_category has values on 4,286 of 12,704 rows and asset_subcategory on 4,286;.',
        "members": ['AST-003'],
    },
    ...
    'ast_008__asset_creation': {
        "desc": 'COUNTS: How many assets were created under a given LSDG theme in a given block for a given year. Returns asset_rows, activities, expenditure. One row per theme by default; other breakdowns on request. expenditure is actual expenditure on the PLAN basis (activity_expenditure), not the cashbook. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Theme mapping covers 17 of 30 focus areas.',
        "members": ['AST-008'],
    },
    ...
    'ast_012__asset_creation': {
        "desc": 'The YEAR-BY-YEAR trend of: How has the number of assets created per year in a given block changed. Returns asset_rows, categorised_assets, activities, expenditure. One row per fiscal_year by default; other breakdowns on request. expenditure is actual expenditure on the PLAN basis (activity_expenditure), not the cashbook. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: activity_asset is sparsely populated: asset_category has values on 4,286 of 12,704 rows and asset_subcategory on 4,286;.',
        "members": ['AST-012'],
    },

    'bud_001__budgeting': {
        "desc": 'TOTALS: How much total funding is recorded for a given gram panchayat in a given year. Returns activities, plan_cost, approved_cost_action_plan, admin_approved_cost, actual_expenditure. One row per gp_name × block_name × fiscal_year. approved_cost_action_plan is the action-plan approved cost. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Funding RECORDED against each GP, one row per GP. The total estimated (planned) cost of planned activities is BUD-006. Caveat: There is no funds-available/receipts table at GP level tied to the plan.',
        "members": ['BUD-001'],
    },
    ...
    'bud_006__budgeting': {
        "desc": "TOTALS: How much planned expenditure is allocated to each GPDP theme in a given year. Returns activities, planned_cost, approved_cost, actual_expenditure. One row per theme by default; other breakdowns on request. planned_cost is the planned cost the GP entered in the action plan. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. PLAN TYPE is a filter on ACTIVITIES, not on plans: 'planned cost of the main GPDPs' is this entry with plan_type bound. 'The total estimated cost / budget outlay of all planned activities, state-wide' is THIS with the total breakdown: planned_cost IS the estimated cost. Caveat: Theme mapping covers 17 of 30 focus areas.",
        "members": ['BUD-006', 'BUD-013'],
    },

    'bud_007__budgeting': {
        "desc": "RANKS: Which GPDP theme has the highest planned expenditure in a given year. Returns planned_cost, activities. One row per theme by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. planned_cost is the planned cost the GP entered in the action plan. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Theme mapping is partial.",
        "members": ['BUD-007'],
    },
    ...
    'bud_012__budgeting': {
        "desc": 'LISTS: Which GPDP themes have no planned expenditure in a given year. Returns activities, planned_cost. One row per theme by default; other breakdowns on request. planned_cost is the planned cost the GP entered in the action plan. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Only themes that appear in the plan are considered;.',
        "members": ['BUD-012'],
    },
    ...
    'dss_001__prioritisation': {
        "desc": "RANKS: Which focus areas in a given gram panchayat have high approved cost but completion below a given threshold percent in a given year. Returns activities, approved_cost, expenditure, completed, completion_rate_pct. One row per focus_area_name by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. expenditure is actual expenditure on the PLAN basis (activity_expenditure), not the cashbook. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Only 17 activities database-wide are complete, so almost every focus area falls below any threshold.",
        "members": ['DSS-001'],
    },

    'dss_002__prioritisation': {
        "desc": "RANKS: Which blocks in a given district combine high pending sanctions with low expenditure in a given year. Returns activities, pending_sanctions, pct_pending, planned_cost, expenditure, pct_utilised. One row per block_name × district_name by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. planned_cost is the planned cost the GP entered in the action plan. Filterable by district; answers state-wide when no place is named — one entry serves every scope. Caveat: 'High' and 'low' are undefined;.",
        "members": ['DSS-002'],
    },
    ...
    'dss_006__prioritisation': {
        "desc": "RANKS: Which GPs in a given block have the largest unspent balance in a given year. Returns activities, approved_cost, expenditure, unspent_balance, pct_utilised. One row per gp_name × block_name by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. expenditure is actual expenditure on the PLAN basis (activity_expenditure), not the cashbook. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: There is no resource-envelope table, so 'unprogrammed envelope balance' is approximated by approved plan cost minus actual expenditure.",
        "members": ['DSS-006'],
    },
    ...
    'exp_012__expenditure': {
        "desc": "RANKS: Which funding source has the highest utilisation in a given year. Returns planned_cost, expenditure, unspent_amount, pct_utilised. One row per funding_source by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. planned_cost is the planned cost the GP entered in the action plan. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Funding source proxied by scheme_name.",
        "members": ['EXP-012'],
    },
    ...
    'exp_014__expenditure': {
        "desc": 'TOTALS: What is the total actual expenditure under each GPDP theme in a given year. Returns activities, planned_cost, actual_expenditure. One row per theme by default; other breakdowns on request. planned_cost is the planned cost the GP entered in the action plan. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Theme mapping covers 17 of 30 focus areas.',
        "members": ['EXP-014'],
    },

    'exp_015__expenditure': {
        "desc": "RANKS: Which GPDP theme has the highest actual expenditure in a given year. Returns actual_expenditure, activities. One row per theme by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. actual_expenditure is actual expenditure on the PLAN basis (activity_expenditure), not the cashbook. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Theme mapping is partial.",
        "members": ['EXP-015'],
    },
    ...
    'exp_018__expenditure': {
        "desc": "RANKS: Which GPDP themes have the highest expenditure utilisation in a given year. Returns planned_cost, actual_expenditure, pct_utilised. One row per theme by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. planned_cost is the planned cost the GP entered in the action plan. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Theme mapping is partial.",
        "members": ['EXP-018'],
    },

    'exp_019__expenditure': {
        "desc": "RANKS: Which GPDP themes have the largest gap between planned and actual expenditure in a given year. Returns planned_cost, actual_expenditure, gap_amount, gap_pct. One row per theme by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. planned_cost is the planned cost the GP entered in the action plan. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Theme mapping is partial.",
        "members": ['EXP-019'],
    },
    ...
    'exp_026__expenditure': {
        "desc": 'COUNTS: How many activities have expenditure under a given focus area in a given year. Returns activities_with_expenditure, total_activities, expenditure. One row per focus_area_name by default; other breakdowns on request. expenditure is actual expenditure on the PLAN basis (activity_expenditure), not the cashbook. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Duplicate of EXP-010 in the source list.',
    ...
    'exp_028__expenditure': {
        "desc": "RANKS: Which focus area has the highest actual expenditure in a given year. Returns actual_expenditure, activities. One row per focus_area_name by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. actual_expenditure is actual expenditure on the PLAN basis (activity_expenditure), not the cashbook. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Set the geography parameters to choose the GP, Block or District level.",
        "members": ['EXP-028'],
    },
    ...
    'exp_038__expenditure': {
        "desc": "TOTALS: How much expenditure went on administrative activities in a given year. Returns activities, planned_cost, actual_expenditure. One row per focus_area_name by default; other breakdowns on request. planned_cost is the planned cost the GP entered in the action plan. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: 'Administrative' is interpreted as the focus areas 'Administrative & Technical Support' and 'GP Office Infrastructure';.",
        "members": ['EXP-038'],
    },
    ...
    'fnd_003__tied_untied_funds': {
        "desc": "COUNTS: How many activities in a given gram panchayat are funded entirely from untied funds in a given year. Returns untied_only_activities, tied_activities, other_component_activities, sanctioned_activities, untied_amount. A single summary row by default; breakdowns on request. Restricted to activities that have an administrative approval (17% of them). Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: 'Entirely' is taken from the dominant component on the activity.",
        "members": ['FND-003'],
    },

    'fnd_004__tied_untied_funds': {
        "desc": 'The PERCENTAGE for: What percentage of sanctioned funds in a given district is tied in a given year. Returns total_sanctioned, tied_amount, untied_amount, other_amount, pct_tied. A single summary row by default; breakdowns on request. Restricted to activities that have an administrative approval (17% of them). Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Tied/untied comes from admin_approval_scheme.scheme_component_code: 4249 = Tied Grant, 4211 = Basic Grant (untied), 4250 = Devolution of Fund (treated as untied).',
        "members": ['FND-004'],
    },

    'fnd_005__category_funds': {
        "desc": 'TOTALS: How much of the sanctioned funding in a given block is earmarked for SC and ST categories in a given year. Returns sanctioned_activities, general_sanctioned, sc_sanctioned, st_sanctioned, total_sanctioned, pct_sc_st. A single summary row by default; breakdowns on request. Restricted to activities that have an administrative approval (17% of them). Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Now uses the real earmark columns from admin_approval_scheme rather than the spent split.',
        "members": ['FND-005'],
    },

    'fnd_006__category_funds': {
        "desc": 'COMPARES: How does the SC-category funding of a given gram panchayat compare with its total for a given year. Returns sc_targeted_activities, sc_amount, total_amount, pct_sc. One row per gp_name × block_name by default; other breakdowns on request. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: No resource-envelope or allocation table exists in the database - only actual expenditure is recorded, so planned allocation cannot be compared against it.',
        "members": ['FND-006'],
    },

    'fnd_007__category_funds': {
        "desc": 'LISTS: Which GPs in a given block have sanctioned activities but no SC/ST earmark in a given year. Returns sanctioned_activities, total_sanctioned, sc_st_sanctioned. One row per gp_name × block_name × district_name by default; other breakdowns on request. Restricted to activities that have an administrative approval (17% of them). Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Still no resource-envelope table, so this lists GPs with zero SC/ST earmark rather than GPs that had an envelope and did not use it.',
        "members": ['FND-007'],
    },
    ...
    'fnd_009__multi_scheme': {
        "desc": "RANKS: Which scheme funds the largest number of activities in a given block in a given year. Returns activities, expenditure. One row per scheme_name by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. expenditure is actual expenditure on the PLAN basis (activity_expenditure), not the cashbook. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: scheme_name is NULL on 82% of expenditure rows;.",
        "members": ['FND-009'],
    },
    ...
    'imp_006__implementation': {
        "desc": "RANKS: Which themes have the highest number of completed activities in a given year. Returns planned_activities, completed_activities, completion_rate_pct. One row per theme by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Progress is read from activity_status.",
        "members": ['IMP-006'],
    },

    'imp_007__implementation': {
        "desc": "RANKS: Which themes have the largest implementation gap (planned versus initiated) in a given year. Returns planned_activities, initiated_activities, implementation_gap, gap_pct. One row per theme by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Progress is read from activity_status.",
        "members": ['IMP-007'],
    },

    'imp_008__implementation': {
        "desc": "RANKS: Which focus area has the highest number of completed activities in a given year. Returns planned_activities, completed_activities. One row per focus_area_name by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Progress is read from activity_status.",
        "members": ['IMP-008'],
    },
    ...
    'imp_010__implementation': {
        "desc": "RANKS: Which focus areas have the largest implementation gap (planned versus initiated) in a given year. Returns planned_activities, initiated_activities, implementation_gap, gap_pct. One row per focus_area_name by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Progress is read from activity_status.",
        "members": ['IMP-010'],
    },
    ...
    'imp_012__implementation': {
        "desc": "RANKS: Which themes receive funds but show poor implementation in a given year. Returns planned_activities, expenditure, completed_activities, completion_rate_pct. One row per theme by default; other breakdowns on request. expenditure is actual expenditure on the PLAN basis (activity_expenditure), not the cashbook. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: 'Poor implementation' is undefined in the source question;.",
        "members": ['IMP-012'],
    },
    ...
    'imp_015__implementation': {
        "desc": "The YEAR-BY-YEAR trend of: Which GPDP themes consistently perform well in implementation across years. Returns years_present, total_activities, started, completed, avg_initiation_rate_pct. One row per theme by default; other breakdowns on request. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: 'Consistently' is not defined in the source question;.",
        "members": ['IMP-015'],
    },

    'imp_016__implementation': {
        "desc": "The YEAR-BY-YEAR trend of: Which GPDP themes consistently underperform in implementation across years. Returns years_present, total_activities, started, completed, avg_initiation_rate_pct. One row per theme by default; other breakdowns on request. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: 'Consistently' is not defined in the source question;.",
        "members": ['IMP-016'],
    },
    ...
    'imp_022__implementation': {
        "desc": "RANKS: Which schemes have the highest number of delayed or incomplete activities in a given year. Returns activities, incomplete_activities, ongoing, abandoned, pct_incomplete. One row per scheme_name by default; other breakdowns on request. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: No planned end-date exists, so 'delayed' cannot be measured;.",
        "members": ['IMP-022'],
    },
    ...
    'phy_003__asset_stages': {
        "desc": "COUNTS: How many assets in a given gram panchayat belong to completed activities in a given year. Returns asset_rows, assets_under_completed_activities. One row per gp_name × block_name by default; other breakdowns on request. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: There is no per-asset completion flag, so the parent activity's status is used instead.",
        "members": ['PHY-003'],
    },
    ...
    'pln_008__planning': {
        "desc": 'COUNTS: How many GPs in a given block uploaded the GPDP after the deadline a given deadline in a given year. Returns gps_late. A single summary row by default; breakdowns on request. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: No upload-date column exists;.',
    ...
    'pln_014__planning': {
        "desc": 'COUNTS: How many Gram Panchayats in a given block/a given district are still awaiting GPDP approval in a given year. Returns gps_awaiting_approval. A single summary row by default; breakdowns on request. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Returns 0 because every plan row carries an approval_date.',
        "members": ['PLN-014'],
    },

    'pln_015__planning': {
        "desc": 'The PERCENTAGE for: What is the GPDP approval rate for each Block in a given year. Returns gps_with_plan, gps_approved, approval_rate_pct. One row per block_name by default; other breakdowns on request. Restricted to plans with an approval date. Filterable by district; answers state-wide when no place is named — one entry serves every scope. Caveat: Approval proxied by approval_date, so the rate is 100% everywhere.',
        "members": ['PLN-015'],
    },

    'pln_016__planning': {
        "desc": 'The PERCENTAGE for: What is the GPDP approval rate for each District in a given year. Returns gps_with_plan, gps_approved, approval_rate_pct. One row per district_name by default; other breakdowns on request. Restricted to plans with an approval date. Filterable by district; answers state-wide when no place is named — one entry serves every scope. Caveat: Approval proxied by approval_date, so the rate is 100% everywhere.',
        "members": ['PLN-016'],
    },
    ...
    'pln_020__planning': {
        "desc": "RANKS: Which Blocks have the highest number of pending GPDP approvals in a given year. Returns pending_approvals, gps_with_plan. One row per block_name by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. Filterable by district; answers state-wide when no place is named — one entry serves every scope. Caveat: pending_approvals is 0 everywhere because approval_date is always populated.",
        "members": ['PLN-020'],
    },

    'pln_021__planning': {
        "desc": "RANKS: Which district have the highest number of pending GPDP approvals in a given year. Returns pending_approvals, gps_with_plan. One row per district_name by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. Filterable by district; answers state-wide when no place is named — one entry serves every scope. RANKS districts by their pending approvals. 'How many GPs still need their GPDP approved' is a COUNT of GPs, PLN-014. Caveat: pending_approvals is 0 everywhere because approval_date is always populated.",
        "members": ['PLN-021'],
    },

    'pln_024__planning': {
        "desc": "COUNTS: How many activities are planned under each GPDP theme in a given gram panchayat in a given year. Returns planned_activities, planned_cost. One row per theme by default; other breakdowns on request. planned_cost is the planned cost the GP entered in the action plan. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. PLAN TYPE is a filter on ACTIVITIES, not on plans: 'activities in the main GPDP' is this entry with plan_type bound, not a plan-count question. 'The total number of activities planned (in the main GPDPs)' is THIS with the total breakdown, and a 'breakdown / distribution of planned activities by theme' is THIS as it stands. Caveat: Themes come from dim_lsdg_theme, which maps only 17 of 30 focus areas;.",
        "members": ['PLN-024'],
    },

    'pln_025__planning': {
        "desc": "RANKS: Which GPDP theme has the highest number of planned activities in a given gram panchayat in a given year. Returns planned_activities, planned_cost. One row per theme by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. planned_cost is the planned cost the GP entered in the action plan. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Ranks THEMES within one place. 'Which GP has the most planned activities' ranks GRAM PANCHAYATS and is PLN-031. Caveat: Theme mapping covers 17 of 30 focus areas.",
    ...
    'pln_039__planning': {
        "desc": "The YEAR-BY-YEAR trend of: Which themes have shown the greatest increase in planned activities between a second year and a given year. Returns activities_year1, activities_year2, change_in_activities. One row per theme by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: The source question said 'in {Date_Range}';.",
        "members": ['PLN-039'],
    },
    ...
    'pln_072__planning': {
        "desc": "TOTALS: Are the planned activities balanced across themes in a given year. Returns theme, planned_activities, pct_share, even_share_pct. One row per theme. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. A judgement of whether the spread across themes is BALANCED. A plain 'breakdown / distribution of planned activities by theme' is PLN-024. Caveat: Same caveat as PLN-044: 'balanced' is not defined, so shares and the even-split benchmark are returned.",
        "members": ['PLN-072'],
    },
    ...
    'plu_006__low_cost_no_cost_activities': {
        "desc": "COUNTS: How many low-cost activities (below a given threshold rupees) are planned theme-wise in a given year. Returns low_cost_activities, total_activities, pct_low_cost. One row per theme by default; other breakdowns on request. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Pass $threshold = 1000 to reproduce the 'below Rs.",
        "members": ['PLU-006'],
    },
    ...
    'san_001__administrative_approval': {
        "desc": 'COUNTS: How many activities in a given gram panchayat received administrative approval in a given year. Returns admin_approved_activities, total_activities, pct_approved. A single summary row by default; breakdowns on request. Restricted to activities that have an administrative approval (17% of them). Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: There is no approval-date or approval-flag column.',
        "members": ['SAN-001'],
    },

    'san_002__administrative_approval': {
        "desc": 'COUNTS: How many activities in a given block are still awaiting administrative approval in a given year. Returns total_activities, sanctioned, awaiting_sanction, cost_recorded_but_no_approval_row, cost_awaiting. A single summary row by default; breakdowns on request. Restricted to activities that have an administrative approval (17% of them). Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Now based on the presence of an admin_approval row rather than a non-zero cost.',
        "members": ['SAN-002'],
    },
    ...
    'san_007__administrative_approval': {
        "desc": 'The PERCENTAGE for: What percentage of planned activities in a given block have received administrative approval in a given year. Returns planned_activities, approved_activities, pct_approved. One row per block_name by default; other breakdowns on request. Restricted to activities that have an administrative approval (17% of them). Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Approval proxied by admin_approved_cost > 0.',
        "members": ['SAN-007'],
    },
    ...
    'san_012__administrative_approval': {
        "desc": "RANKS: Which GPs in a given block have the highest total proposed cost awaiting administrative sanction in a given year. Returns activities_awaiting, proposed_cost_awaiting, sanctioned_activities, sanctioned_amount. One row per gp_name × block_name by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. Restricted to activities that have an administrative approval (17% of them). Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: 'Awaiting sanction' now means no row in admin_approval.",
        "members": ['SAN-012'],
    },
    ...
    'sbm_gwm_004__grey_water_management': {
        "desc": 'COUNTS: How many community soak pits have been approved in a given year. Returns matching_activities, approved_activities, planned_cost, total_expenditure. A single summary row by default; breakdowns on request. Identifies the activity by KEYWORD MATCH on activity_name + activity_desc against the pattern /(soak).*(community|group|cluster)|(community|group|cluster).*(soak)/. Nothing in the database codes SBM activity types, so this is a text search: it both misses differently-worded activities and picks up unrelated ones. Counts ONE item type by keyword. A question about sanitation activities in general is not this: a count of PLANNED sanitation activities is PLN-049, a count by status (completed, ongoing) is STS-003. Restricted to activities that have an administrative approval (17% of them). planned_cost is the planned cost the GP entered in the action plan. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: SBM activity types are not coded anywhere in the database - asset_subcategory is missing on two-thirds of asset rows and asset_name is 100% NULL.',
        "members": ['SBM-GWM-004'],
    },
    ...
    'sbm_om_010__operation_maintenance': {
        "desc": 'TOTALS: What is the total Operation & Maintenance expenditure in a given gram panchayat in a given year. Returns maintenance_activities, planned_cost, om_expenditure. One row per focus_area_name by default; other breakdowns on request. planned_cost is the planned cost the GP entered in the action plan. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: O&M is read as work_type Maintenance or Upgradation.',
        "members": ['SBM-OM-010'],
    },
    ...
    'sch_001__scheme_coverage': {
        "desc": 'COUNTS: How many activities are recorded under a given scheme in a given district for a given year. Returns activities, expenditure. One row per scheme_name by default; other breakdowns on request. expenditure is actual expenditure on the PLAN basis (activity_expenditure), not the cashbook. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: scheme_name has only 5 non-null values and is NULL on 82% of rows.',
        "members": ['SCH-001'],
    },
    ...
    'sch_009__scheme_coverage': {
        "desc": 'COUNTS: What is the status breakdown of activities under a given scheme in a given district for a given year. Returns activities, expenditure. One row per status_label by default; other breakdowns on request. expenditure is actual expenditure on the PLAN basis (activity_expenditure), not the cashbook. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: scheme_name coverage is 18%;.',
        "members": ['SCH-009'],
    },

    'sch_010__scheme_coverage': {
        "desc": 'TOTALS: What is the General/SC/ST funding split under a given scheme in a given block for a given year. Returns general_amount, sc_amount, st_amount, total_amount. One row per scheme_name by default; other breakdowns on request. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: sc and st amounts are sparsely populated.',
        "members": ['SCH-010'],
    },

    'sch_011__scheme_coverage': {
        "desc": 'COUNTS: What is the district-wise activity count under a given scheme for a given year. Returns activities, expenditure. One row per district_name by default; other breakdowns on request. expenditure is actual expenditure on the PLAN basis (activity_expenditure), not the cashbook. Caveat: scheme_name coverage is 18%.',
        "members": ['SCH-011'],
    },

    'sts_001__status_counts': {
        "desc": "COUNTS: How many activities in a given gram panchayat are in each progress status for a given year. Returns activities, planned_cost, expenditure. One row per status_label by default; other breakdowns on request. planned_cost is the planned cost the GP entered in the action plan. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Splits activities across ALL statuses. A count of ONE status (completed, ongoing) is STS-003 or IMP-002. Caveat: activity_status code 173 decodes to 'Buildings' in dim_code, which is not a status and needs verifying.",
        "members": ['STS-001'],
    },
    ...
    'sts_006__status_counts': {
        "desc": 'The PERCENTAGE for: What percentage of taken-up activities in a given block are completed in a given year. Returns taken_up_activities, completed_activities, pct_completed. A single summary row by default; breakdowns on request. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Progress is read from activity_status.',
        "members": ['STS-006'],
    },

    'sts_007__status_counts': {
        "desc": "RANKS: Which blocks in a given district have the highest activity completion rate for a given year. Returns activities, started, completed, pct_completed. One row per block_name × district_name by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. Filterable by district; answers state-wide when no place is named — one entry serves every scope. Caveat: Progress is read from activity_status.",
        "members": ['STS-007'],
    },

    'sts_008__status_counts': {
        "desc": "The PERCENTAGE for: What share of approved activities in a given district has not yet started in a given year. Returns activities, approved_not_started, started, pct_not_started. A single summary row by default; breakdowns on request. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: 'Not started' is read as status 'Activity Approved' - approved but with no work status recorded.",
        "members": ['STS-008'],
    },

    'sts_009__status_counts': {
        "desc": 'LISTS: Which GPs in a given block have zero completed activities in a given year. Returns activities, completed. One row per gp_name × block_name × district_name by default; other breakdowns on request. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Progress is read from activity_status.',
        "members": ['STS-009'],
    },

    'sts_010__status_counts': {
        "desc": 'COUNTS: How many activities in a given block are stuck in Under Approval status for a given year. Returns under_approval, total_activities, cost_under_approval. A single summary row by default; breakdowns on request. Counts only activities under approval. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: 36 activities carry this status across the whole database.',
        "members": ['STS-010'],
    },

    'sts_011__status_counts': {
        "desc": 'LISTS: Which blocks in a given district have every taken-up activity started for a given year. Returns activities, started, pct_started. One row per block_name × district_name by default; other breakdowns on request. Filterable by district; answers state-wide when no place is named — one entry serves every scope. Caveat: Progress is read from activity_status.',
        "members": ['STS-011'],
    },
    ...
    'trd_005__year_on_year': {
        "desc": 'The YEAR-BY-YEAR trend of: How has the activity completion rate of a given district changed over the years. Returns activities, started, completed, completion_rate_pct, initiation_rate_pct. One row per fiscal_year by default; other breakdowns on request. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Only 17 activities in the whole database are marked WORK COMPLETED, so completion rates are near zero throughout.',
        "members": ['TRD-005'],
    },
    ...
    'trd_007__physical_vs_financial': {
        "desc": 'COMPARES: What are the approved cost, expenditure and status counts theme-wise for a given gram panchayat in a given year. Returns activities, approved_cost, expenditure, started, ongoing, completed. One row per theme by default; other breakdowns on request. expenditure is actual expenditure on the PLAN basis (activity_expenditure), not the cashbook. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Theme mapping is partial.',
        "members": ['TRD-007'],
    },

    'trd_008__physical_vs_financial': {
        "desc": "COMPARES: What are the approved cost and expenditure sector-wise in a given district for a given year. Returns activities, approved_cost, expenditure, pct_utilised. One row per sector by default; other breakdowns on request. expenditure is actual expenditure on the PLAN basis (activity_expenditure), not the cashbook. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Approved cost AND expenditure per focus area. 'Tied-fund expenditure, water vs sanitation' is EXP-009, which reports tied spend per focus area. Caveat: 'Sector' is read as focus area, the closest sectoral classification in the data.",
        "members": ['TRD-008'],
    },

    'trd_009__physical_vs_financial': {
        "desc": "RANKS: Which themes in a given block show high expenditure but low activity completion in a given year. Returns activities, expenditure, completed, completion_rate_pct. One row per theme by default; other breakdowns on request. expenditure is actual expenditure on the PLAN basis (activity_expenditure), not the cashbook. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: 'High' and 'low' are undefined;.",
        "members": ['TRD-009'],
    },
    ...
    'trd_011__entity_comparison': {
        "desc": 'COMPARES: Compare activity counts, expenditure and completion rates between a given block and a second block for a given year. Returns gps, activities, planned_cost, actual_expenditure, pct_utilised, completion_rate_pct. One row per block_name × district_name by default; other breakdowns on request. planned_cost is the planned cost the GP entered in the action plan. A two-BLOCK head-to-head. Both blocks must be named. Caveat: Both block parameters must be supplied.',
        "members": ['TRD-011'],
    },
