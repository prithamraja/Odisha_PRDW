...
FAMILY_DESCRIPTIONS: dict[str, dict] = {
    ...
    'pln_021__planning': {
        "desc": "RANKS: Which district have the highest number of pending GPDP approvals in a given year. Returns pending_approvals, gps_with_plan. One row per district_name by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. Filterable by district; answers state-wide when no place is named — one entry serves every scope. Caveat: pending_approvals is 0 everywhere because approval_date is always populated.",
        "members": ['PLN-021'],
    },
    ...
    'sbm_gwm_004__grey_water_management': {
        "desc": 'COUNTS: How many community soak pits have been approved in a given year. Returns matching_activities, approved_activities, planned_cost, total_expenditure. A single summary row by default; breakdowns on request. Identifies the activity by KEYWORD MATCH on activity_name + activity_desc against the pattern /(soak).*(community|group|cluster)|(community|group|cluster).*(soak)/. Nothing in the database codes SBM activity types, so this is a text search: it both misses differently-worded activities and picks up unrelated ones. Counts ONE item type by keyword. A question about sanitation activities in general, or a status count, is STS-003. Restricted to activities that have an administrative approval (17% of them). planned_cost is the planned cost the GP entered in the action plan. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: SBM activity types are not coded anywhere in the database - asset_subcategory is missing on two-thirds of asset rows and asset_name is 100% NULL.',
        "members": ['SBM-GWM-004'],
    },
    ...
    'trd_002__year_on_year': {
        "desc": 'COMPARES: Compare the approved cost and expenditure theme-wise between a second year and a given year. Returns approved_cost_year1, approved_cost_year2, expenditure_year1, expenditure_year2. One row per theme by default; other breakdowns on request. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Compares TWO DIFFERENT named years side by side. Tied-fund spending on water vs sanitation within one period is EXP-009. Caveat: Theme mapping is partial.',
        "members": ['TRD-002'],
    },
