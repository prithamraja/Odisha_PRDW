...
FAMILY_DESCRIPTIONS: dict[str, dict] = {
    ...
    'ast_001__asset_creation': {
        "desc": 'COUNTS: How many assets were created in a given gram panchayat during a given year. Returns asset_rows, asset_creating_activities, categorised_assets. One row per gp_name × block_name by default; other breakdowns on request. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: activity_asset is sparsely populated: asset_category has values on 4,286 of 12,704 rows and asset_subcategory on 4,286;.',
        "members": ['AST-001'],
    },
    ...
    'bud_006__budgeting': {
        "desc": "TOTALS: How much planned expenditure is allocated to each GPDP theme in a given year. Returns activities, planned_cost, approved_cost, actual_expenditure. One row per theme by default; other breakdowns on request. planned_cost is the planned cost the GP entered in the action plan. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. PLAN TYPE is a filter on ACTIVITIES, not on plans: 'planned cost of the main GPDPs' is this entry with plan_type bound. Caveat: Theme mapping covers 17 of 30 focus areas.",
    ...
    'exp_002__expenditure': {
        "desc": 'The YEAR-BY-YEAR trend of: How has the total actual expenditure of a given gram panchayat changed over the years. Returns activities, planned_cost, actual_expenditure, pct_utilised. One row per fiscal_year by default; other breakdowns on request. planned_cost is the planned cost the GP entered in the action plan. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Six years are present: 2020-2021 to 2025-2026.',
        "members": ['EXP-002'],
    },
    ...
    'exp_023__expenditure': {
        "desc": 'The PERCENTAGE for: What percentage of sanctioned funds was utilised in a given year. Returns admin_sanctioned, actual_expenditure, pct_of_sanctioned_utilised. A single summary row by default; breakdowns on request. actual_expenditure is actual expenditure on the PLAN basis (activity_expenditure), not the cashbook. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: admin_approved_cost is populated on only 2,247 of 12,730 expenditure rows, so the denominator is incomplete.',
        "members": ['EXP-023'],
    },
    ...
    'pln_024__planning': {
        "desc": "COUNTS: How many activities are planned under each GPDP theme in a given gram panchayat in a given year. Returns planned_activities, planned_cost. One row per theme by default; other breakdowns on request. planned_cost is the planned cost the GP entered in the action plan. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. PLAN TYPE is a filter on ACTIVITIES, not on plans: 'activities in the main GPDP' is this entry with plan_type bound, not a plan-count question. Caveat: Themes come from dim_lsdg_theme, which maps only 17 of 30 focus areas;.",
        "members": ['PLN-024'],
    },

    'pln_025__planning': {
        "desc": "RANKS: Which GPDP theme has the highest number of planned activities in a given gram panchayat in a given year. Returns planned_activities, planned_cost. One row per theme by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. planned_cost is the planned cost the GP entered in the action plan. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Theme mapping covers 17 of 30 focus areas.",
    ...
    'pln_031__planning': {
        "desc": "RANKS: Which Gram Panchayats have planned the highest number of activities under a given LSDG theme in a given year. Returns planned_activities, planned_cost. One row per gp_name × block_name × district_name by default; other breakdowns on request. A top-N list, highest first; $top_n = 1 answers 'which is the single highest/lowest'. planned_cost is the planned cost the GP entered in the action plan. Filterable by district, block; answers state-wide when no place is named — one entry serves every scope. Caveat: Theme values must match dim_lsdg_theme.lsdg_theme exactly (note the trailing space on some).",
        "members": ['PLN-031'],
    },
    ...
    'plu_003__plan_structure': {
        "desc": 'LOOKS UP: Does a given gram panchayat have a supplementary plan in addition to the main plan for a given year. Returns main_plans, supplementary_plans, has_supplementary. One row per gp_name × block_name × district_name by default; other breakdowns on request. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope.',
        "members": ['PLU-003'],
    },

    'plu_004__plan_structure': {
        "desc": 'COUNTS: How many GPs in a given block uploaded supplementary plans for a given year. Returns gps_with_supplementary, supplementary_plans. A single summary row by default; breakdowns on request. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope.',
        "members": ['PLU-004'],
    },
    ...
    'sts_001__status_counts': {
        "desc": "COUNTS: How many activities in a given gram panchayat are in each progress status for a given year. Returns activities, planned_cost, expenditure. One row per status_label by default; other breakdowns on request. planned_cost is the planned cost the GP entered in the action plan. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: activity_status code 173 decodes to 'Buildings' in dim_code, which is not a status and needs verifying.",
        "members": ['STS-001'],
    },
    ...
    'sts_013__status_counts': {
        "desc": 'COUNTS: What is the district-wise activity status summary for a given year. Returns activities, approved_not_started, ongoing, completed, abandoned, under_approval. One row per district_name by default; other breakdowns on request. Caveat: Progress is read from activity_status.',
        "members": ['STS-013'],
    },
    ...
    'trd_002__year_on_year': {
        "desc": 'COMPARES: Compare the approved cost and expenditure theme-wise between a second year and a given year. Returns approved_cost_year1, approved_cost_year2, expenditure_year1, expenditure_year2. One row per theme by default; other breakdowns on request. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Theme mapping is partial.',
        "members": ['TRD-002'],
    },

    'trd_003__year_on_year': {
        "desc": 'The YEAR-BY-YEAR trend of: What is the year-wise expenditure of a given gram panchayat against the plan of each year. Returns activities, approved_cost, expenditure, pct_utilised. One row per fiscal_year by default; other breakdowns on request. expenditure is actual expenditure on the PLAN basis (activity_expenditure), not the cashbook. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: Expenditure is recorded against the year of the plan it belongs to;.',
        "members": ['TRD-003'],
    },
    ...
    'trd_008__physical_vs_financial': {
        "desc": "COMPARES: What are the approved cost and expenditure sector-wise in a given district for a given year. Returns activities, approved_cost, expenditure, pct_utilised. One row per sector by default; other breakdowns on request. expenditure is actual expenditure on the PLAN basis (activity_expenditure), not the cashbook. Filterable by district, block, GP; answers state-wide when no place is named — one entry serves every scope. Caveat: 'Sector' is read as focus area, the closest sectoral classification in the data.",
        "members": ['TRD-008'],
    },
