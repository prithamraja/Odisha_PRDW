...
class CaveatPassthroughTests(_AdapterFixture):
    ...
    def test_the_caveat_is_both_a_field_and_appended_to_the_answer(self):
        """DELIBERATE CHANGE OF BEHAVIOUR IN WP-3 — this test asserted the
        opposite at WP-1 (`assertNotIn(CAVEAT, echo_answer(result))`).

        WP-1 kept the caveat out of the answer text on the reasoning that answer
        text can be regenerated by an LLM and a glued-in caveat could be
        paraphrased away. That reasoning holds for LLM-generated text — and this
        answer is not LLM-generated. `echo_answer` composes it deterministically
        from the resolved question, so appending happens after every step a model
        could touch.

        What tipped it: 296 of the 346 shipped templates carry a caveat, and the
        only consumer that would render `QueryResponse.caveat` as a distinct
        element is a frontend that is a separate, deferred workstream. A caveat
        that exists solely in a field nothing renders is a caveat nobody reads,
        which for 251 Partial questions is the confidently-wrong failure mode
        decision D3 exists to prevent.

        So it is BOTH, and both halves are asserted here: the field is unchanged
        for a client that wants to style it, and the text carries it verbatim for
        one that does not.
        """
        from query_router.echo import echo_answer, echo_answer_without_caveat
        result = self._serve("P001", district_name="Puri")

        self.assertEqual(result.caveat, CAVEAT)
        answer = echo_answer(result)
        self.assertIn(CAVEAT, answer)
        # Verbatim, not summarised, and after the answer rather than inside it.
        self.assertTrue(answer.endswith(CAVEAT))
        self.assertTrue(answer.startswith(echo_answer_without_caveat(result)))

    def test_an_uncaveated_entry_gains_no_note(self):
        from query_router.echo import echo_answer, echo_answer_without_caveat
        result = self._serve("T001", district_name="Puri")
        self.assertEqual(echo_answer(result), echo_answer_without_caveat(result))
        self.assertNotIn("Note:", echo_answer(result))
