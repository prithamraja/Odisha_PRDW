...
| WP-D2 | `VIEW*_CONFIG`s + column glossaries + audience framing; run phase4b→5→5b; D17 model-budget check; calibration on top-15/view. | Operator-as-SME calibration (D20): top-15/view labeled real / already-known / spurious; no nonsense findings in top ranks. Sample validates machinery; re-calibration re-runs when statewide lands. | **Report in (`handoffs/WPD2_REPORT.md`, 2026-08-14): T1/T2/T4 verified green; view2+view3 mined and drained (110+5 candidates → 18 ranked); GPT-5.6 pinned as `gpt-5.6-sol` on probe evidence (3 ids on the live list; budget 9000 re-verified, worst case 6,378 headroom). view1 escalated correctly: 880,752 scopes ≈ 9.6–16.4 h + never-evict cache memory wall — not drained, not scored.** **WP-D2b DONE — gate green** (report `handoffs/WPD2b_REPORT.md`: view1 drained at depth 1 in 262.5 s, 48,792/48,792 scopes, 0.60 GB; all four report checks green over three sections, 70/70 figures traced). **PM replay 2026-08-14 went further: independent re-mine + re-rank on the PM mirror — view1/view3 candidates byte-identical to the recorded hashes, ranking identical, config gate 0 failures, report checks 0 failures against the re-derivation.** One flag: view2's candidate-file hash differs across runs with identical ranked output (D26 → WP-D2c reproducibility item). **Next: the calibration session (operator + PM, 33 findings, package v2 at `handoffs/WPD2_calibration/`).** |
| WP-D2c | Engine scaling + calibration actions (brief next): (a) scaling — parallelize mining across subspaces, cache eviction/spill, candidate streaming (statewide-checklist item pulled forward; start from the measured 185.9 scopes/s); (b) **calibration actions 1–7** from `handoffs/WPD2_calibration/CALIBRATION_SESSION_1.md` — definitional-pair exclusion, twin dedup, EVENNESS/linkage/size reframing rules, per-GP intensity measures, known-events context table, degenerate-measure guard; (c) D26 carryovers — stale phase4b budget line, view2 hash reproducibility, statewide-depth decision; (d) the labeled sheet becomes a **regression gate** (no labeled-spurious class re-enters a top-15). | Full depth-2 sample queue drains on available hardware; determinism check vs single-threaded; re-mine + re-rank shows top-15s clean against the calibration baseline — **this closes WP-D2's workstream gate**. | **Brief ready: `handoffs/WPD2c_engine_scaling.md`** (T1+T2 core close the WP-D2 gate; T3+T4 deliver scaling + the depth-2 sample run with new-at-depth-2 findings flagged for calibration session 2) |
