...
6. **Discover asks (2026-08-13):** (a) statewide `Data/` drop when ready —
   operator confirmed same one-CSV-per-table format (D15); (b) ~~name the
   Discover SME~~ RESOLVED — the operator is the Discover SME (D20);
   (c) commit the Discover baseline (`Insights/`,
   `ODISHA_PRDW_METAINSIGHTS_HANDOFF.md`, this plan update, the WPD0 brief)
   before the first WP-D agent run.
