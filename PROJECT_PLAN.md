...
| WP-4c | F1 fix (D28.1) + §5.2 behavior fixes (D28.3–5) + `usage` instrumentation; **re-run all evals**; then F2 A/B (D28.2, SME-gated); thresholds only from post-F1 evidence with 3× replay proof. | Benchmarks met, or every gap root-caused with evidence; consistency ≈3% now meaningfully measurable. | **Brief ready: `handoffs/WP4c_fix_and_rerun.md`** |
