...
**T2 — D18 catalogue fixes (edit the GENERATOR, then regenerate — never the generated files):**
  ...
  d) **Echo rendering of partially-bound questions** (operator-reported defect, 2026-08-13, screenshot-verified). `router.py` ~1437 builds `query_description` with all-or-nothing `.format(**entity_values)`: one unbound optional slot raises `KeyError` and the fallback prints the RAW abstract question — discarding even the substitutions that were bound (the operator's "each GP in 2024-25" question echoed `{gp_name}` AND `{date_range}` unfilled while executing correctly). Latent AP bug: with all-required slots the except never fired; under D2 partial binding is the normal case. Fix: per-placeholder substitution (`string.Formatter`/`format_map` with a safe mapping) that (i) fills bound slots with resolved display values, (ii) renders unbound OPTIONAL slots with a per-slot collective phrase — `gp_name` → "all GPs" (use "each GP" where the query groups by GP), `district_name` → "all districts", `block_name` → "all blocks", `scheme` → "all schemes", `focus_area` → "all focus areas", `theme` → "all themes", `asset_category`/`asset_sub_category` → "all categories", `top_n` → its default. Tests: bound values render; unbound optional renders its collective phrase; **no `{` ever survives into an answer** (assert over every catalogue entry with zero entities bound — the whole-of-state echo of all 346).

  e) **Fragment tier selection ignores the frame's executable slots** (operator-reported, 2026-08-13, screenshot-verified). "what about Laxmipur?" after a `gp_name`-scoped question: `_bare_fragment_edit` walks `GEO_TIERS_WIDEST_FIRST` and stops at the first tier that VALIDATES (Laxmipur = block), then `drill_target` finds the frame's template (`EXP-001`) has no `block_name` slot → hop abandoned → generic "narrowed or new?" clarify with a name-similarity retrieval chip (the docstring's own predicted failure). Fix: (i) intersect candidate tiers with `_geo_slots_of(frame.template_id)` BEFORE resolving — only executable tiers compete; (ii) exactly one tier resolves+fits → auto-slot and serve the hop (echo names the tier: "Laxmipur (GP)"); (iii) two tiers resolve+fit → TIER clarification with tier-qualified chips per D18.P3 ("Laxmipur — the GP in Koraput" / "Laxmipur block"), never the generic ambiguous-fragment prompt; (iv) in `ambiguous_fragment_clarify`, drop the standalone-fragment chip when `templates_share_subject` fails between it and the frame — a chip whose only tie to the fragment is the place name is noise. Note the clarify prompt also inherits T2(d)'s broken `{placeholder}` echo via `frame_question` — verify T2(d) fixes the stored frame question too. Add 1–2 gold fragment rows covering fragment × tier-collision × slot-constraint (the G1860 pattern at B4's collision), rebuild via `--install`.
