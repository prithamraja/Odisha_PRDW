"""WP-D11b config gate — the WP-D11 four-view gate, plus the averaged twins.

WP-D11b copy of `handoffs/WPD11_calibration/verify_configs_prdw_d11.py`, which is
left untouched as WP-D11's record. Diff from that copy, in full:
  * §5's view4 measure count is 74 (60 SUM + 14 averaged twins, D61 ruling 4),
    and view3's is pinned at 28 (18 SUM + 10 twins).
  * a new §5b checks what makes the twins work end to end: each is an AVG alias
    of a SUM measure of its own view; each is registered as its total's
    intensity companion, so rule 2c FIRES for it (the brief's fact 5); each has
    its total's unit averaged at the view's grain, a formatter that keeps a
    decimal (an averaged count must not print as a false zero) and a glossary
    entry that says AVERAGED; and a twin can never be A2-MERGED with its total,
    because the twin key includes the measure name.
  * §5b also checks the size-band display map (D61 ruling 3): it covers exactly
    the four gp_size values the built view carries, it is idempotent, and it
    leaves figures that merely contain a band's digits alone.

Original WP-D11 header follows.

WP-D11 config gate — the WP-D2 T1/T2 checks, at four views.

THIS IS THE WP-D2 GATE WITH ITS VIEW COUNT CORRECTED, AND NOTHING ELSE MOVED.
`handoffs/WPD2_calibration/verify_configs_prdw.py` asserts in four places that
the deployment holds exactly three views and that no VIEW4_CONFIG survives on
phase4a_engine. Amendment B §12.4 made both statements false on 2026-09-07, so
that file now fails on a change it was written before. It is left exactly as it
is -- it is the WP-D2 gate and it recorded what was true then -- and this copy
carries the same checks against four views. Everything else below, including
every threshold, every AP-vocabulary token and every caveat case, is unchanged.

Diff from the WP-D2 copy, in full:
  * §2 expects four views everywhere, and VIEW4_CONFIG is now REQUIRED on
    phase4a_engine rather than forbidden -- phase5c_gamma_reports.discover_views()
    finds the reportable views by scanning that module, so its absence would
    silently drop view4 from the gamma editions.
  * §3's CFG map gains view4.
  * §5 checks view4's statewide dimension flip and its depth (1 sample / 2
    statewide), and checks that the six sample profile dimensions are present
    on ALL FOUR views while plc_available is added by the statewide branch and
    by nothing else.
  * §5 also pins the one thing Amendment B is most easily broken by: the seven
    bands must be IDENTICAL across the four views at a given scale. A band that
    meant one thing on view1 and another on view4 would make every cross-view
    comparison in the feed wrong.
...
def main() -> int:
    ...
    ALL = ["view1", "view2", "view3", "view4"]
    print("\n=== 2. registries hold exactly the four PR&DW views ===")
    check(list(phase5b_report.VIEW_CONFIGS) == ALL,
          "phase5b_report.VIEW_CONFIGS", str(list(phase5b_report.VIEW_CONFIGS)))
    check(list(phase5b_report.VIEW_DESCRIPTIONS) == ALL,
          "phase5b_report.VIEW_DESCRIPTIONS")
    check(list(phase5b_dual_reports.ALL_CONFIGS) == ALL,
          "phase5b_dual_reports.ALL_CONFIGS")
    check(phase5c_global_feed.VIEWS == ALL, "phase5c_global_feed.VIEWS")
    check(list(phase5c_global_feed.VIEW_TITLES) == ALL,
          "phase5c_global_feed.VIEW_TITLES")
    import phase5d_retrieval_corpus, phase5f_decompose
    check(list(phase5d_retrieval_corpus.VIEWS) == ALL, "phase5d_retrieval_corpus.VIEWS")
    check(list(phase5f_decompose.VIEWS) == ALL, "phase5f_decompose.VIEWS")
    check(list(phase5f_decompose.CONFIGS) == ALL, "phase5f_decompose.CONFIGS")
    # REQUIRED, not forbidden: discover_views() scans this module by name, so
    # VIEW4_CONFIG missing here would drop view4 from every gamma edition
    # without failing anything else.
    check(hasattr(phase4a_engine, "VIEW4_CONFIG"),
          "VIEW4_CONFIG is reachable on phase4a_engine")
    check(not any(re.match(r"^VIEW[5-9]_CONFIG$", a) for a in dir(phase4a_engine)),
          "no VIEW5..9_CONFIG survives on phase4a_engine")
    views, skipped = phase5c_gamma_reports.discover_views()
    check(sorted([v[0] for v in views] + [s[0] for s in skipped]) == ALL,
          "phase5c_gamma_reports.discover_views sees exactly four",
          f"views={[v[0] for v in views]} skipped={skipped}")
    ...
    CFG = {"view1": phase2_engine.VIEW1_CONFIG,
           ...
           "view4": phase2_engine.VIEW4_CONFIG}
    ...
    probe = (
        ...
        " ('view3', p4.VIEW3_CONFIG), ('view4', p2.VIEW4_CONFIG)):\n"
    ...
    # WP-D11: the profile dimensions are the tail of every list, so the
    # geography assertions are made on the head and the bands are checked
    # separately below.
    PROFILE6 = ["social_composition", "gp_size", "remoteness", "digital_readiness",
                "has_panchayat_bhawan", "has_csc"]
    PROFILE7 = PROFILE6 + ["plc_available"]

    def geo(dims):
        return [d for d in dims if d not in PROFILE7]

    check(geo(state["view1"]["dims"]) == ["district_name"] + geo(sample["view1"]["dims"])[3:],
          "view1 statewide drops gp_name and block_name only")
    check(geo(state["view2"]["dims"]) == ["district_name", "block_name", "fiscal_year"],
          "view2 statewide dims")
    check(geo(state["view3"]["dims"]) == ["district_name", "block_name"],
          "view3 statewide dims")
    check(geo(state["view4"]["dims"]) == ["district_name", "block_name"],
          "view4 statewide dims")
    check(geo(sample["view4"]["dims"]) == ["gp_name", "block_name", "district_name"],
          "view4 sample dims")

    # THE AMENDMENT B INVARIANT. Same seven bands, same order, on all four
    # views at a given scale -- six at sample scale, seven statewide, and
    # plc_available added by the statewide branch and by nothing else. A band
    # that appeared on some views and not others would make the feed's
    # cross-view comparisons quietly wrong.
    for n in CFG:
        check([d for d in sample[n]["dims"] if d in PROFILE7] == PROFILE6,
              f"{n}: the six sample profile bands, in order")
        check([d for d in state[n]["dims"] if d in PROFILE7] == PROFILE7,
              f"{n}: all seven profile bands statewide, in order")
    check(sample["view4"]["impact"] == ["population_total", "n_activities"],
          "view4 impact measures are B3's two")
    check(len(sample["view4"]["measures"]) == 74,
          "view4 carries 60 SUM measures + 14 averaged twins (WP-D11b)",
          str(len(sample["view4"]["measures"])))
    check(len(sample["view3"]["measures"]) == 28,
          "view3 carries 18 SUM measures + 10 averaged twins (WP-D11b)",
          str(len(sample["view3"]["measures"])))
    ...
    check(sample["view4"]["depth"] == 1 and state["view4"]["depth"] == 2,
          "view4 depth 1 sample / 2 statewide (§12.4)")

    print("\n=== 5b. WP-D11b: averaged twins and size-band display ===")
    import inspect
    p5b = phase5b_report
    for n, want in (("view3", 10), ("view4", 14)):
        cfg = CFG[n]
        avg = [m for m in cfg.measures if m.agg == "avg"]
        check(len(avg) == want, f"{n}: {want} averaged twins", str(len(avg)))
        check(all(cfg.get_agg(m.column) == "sum" for m in avg),
              f"{n}: every twin averages a SUM measure of this view")
        comp = p5b._INTENSITY_COMPANION.get(n, {})
        check(all(comp.get(m.column) == m.name for m in avg),
              f"{n}: every twin is its total's intensity companion (rule 2c fires)")
        units = p5b._UNITS[n]
        check(all(units[m.name] == p5b.averaged_unit(units[m.column], n) for m in avg),
              f"{n}: every twin's unit is its total's, averaged at the view's grain")
        gloss = p5b.VIEW_DESCRIPTIONS[n]["column_glossary"]
        check(all("AVERAGED" in gloss[m.name] and m.column in gloss[m.name]
                  for m in avg),
              f"{n}: every twin's glossary entry says AVERAGED and names its total")
        shown = [p5b.format_measure(n, m.name, 0.35) for m in avg]
        check(all(p5b.AVERAGED_GRAIN[n] in s for s in shown),
              f"{n}: every twin formats with its grain", shown[0])
        check(not [s for s in shown if s.startswith("0 ")],
              f"{n}: an averaged count keeps its decimal (no false zero)",
              str([s for s in shown if s.startswith("0 ")]))
        check(p5b.activity_count_measures(n) >= {"n_activities", "n_activities_mean"},
              f"{n}: the FY 2023-24 caveat still scopes the averaged activity count")
    check(p5b._INTENSITY_COMPANION["view2"] == {
              "payment_amount": "payment_amount_mean",
              "receipt_amount": "receipt_amount_mean"},
          "view2's two companions are exactly as WP-D2c shipped them")
    import phase5_ranking
    check("mi.measure" in inspect.getsource(phase5_ranking._twin_key),
          "a twin and its total can never be A2-merged (the twin key has the measure)")

    bd = p5b.BAND_DISPLAY
    try:
        import pandas as pd
        vals = set(pd.read_parquet(CFG["view4"].parquet_path)["gp_size"].dropna())
        check(vals - {"Not reported"} == set(bd),
              "the band map covers exactly the gp_size values the view carries",
              str(sorted(vals)))
    except Exception as exc:                                   # no parquet here
        print(f"  [skip] view4 parquet not readable here ({type(exc).__name__})")
    once = {k: p5b.display_band_labels(k) for k in bd}
    check(all(once[k] == v for k, v in bd.items()), "every band label maps to its display form")
    check(all(p5b.display_band_labels(v) == v for v in bd.values()),
          "the map is idempotent (a displayed label is left alone)")
    for untouched in ("12,500 to 15,000", "Rs 2,500", "10,000 activities",
                      "under 2,500 people"):
        check(p5b.display_band_labels(untouched) == untouched,
              f"the map leaves {untouched!r} alone")
